            ...
            dir('Backend') {
              script {
                ...
                withSonarQubeEnv('SonarQube') {
                  sh "${sonarBuild}/SonarScanner.MSBuild.exe begin /k:${SONAR_PROJECT_KEY} /d:sonar.host.url=${SONAR_SERVER_URL} /d:sonar.login=${SONAR_TOKEN_BACK}"
                  sh 'dotnet build'
                  sh "${sonarBuild}/SonarScanner.MSBuild.exe end /d:sonar.login=${SONAR_TOKEN_BACK}"
                }
              }
            }
          }
